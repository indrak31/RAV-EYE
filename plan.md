...
- [ ] `GET /api/cases` — filters: `status` (csv), `camera_id`, `violation_type`, `plate` (substring), `limit`/`offset`. Returns `{ total, limit, offset, items[] }` with evidence stripped. **Uses `case_service.list_cases()`.**
- [ ] `GET /api/cases/{case_id}` — full Case with `evidence[]` populated. `404 case_not_found` on miss.
- [ ] `PATCH /api/cases/{case_id}/review` — body `{ decision, reviewer_id, note }`. Sets `status` to `approved`/`rejected`, stamps `reviewed_at`. `409 case_not_reviewable` if Case not in `ingested`/`in_review`. **Uses `case_service.review_case()`.**
...
- [ ] `POST /api/challan` — returns 202 with `{ challan_id, status: "queued" }`. Refuses if target Case `status != approved` (-> `409 challan_not_eligible`). Mock service writes a `challans` table row; no real SMS/email. **Uses `case_service.is_challan_eligible()` + `mark_challan_issued()`.**
...
- [ ] Tests: challan 202 / 409, WS event subscription.
